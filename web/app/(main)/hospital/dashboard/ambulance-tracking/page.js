    ...
    <div className="p-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: "#64748b" }}>Live Tracking</span>
        </div>
        <h1 className="text-3xl font-extrabold" style={{ color: "#0f172a" }}>🚑 Ambulance Tracking</h1>
      ...
      {loading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <p className="text-lg font-bold" style={{ color: "#0f172a" }}>No ambulances currently</p>
          <p className="text-sm mt-1" style={{ color: "#94a3b8" }}>Ambulances assigned to your hospital will appear here in real-time.</p>
        </motion.div>
      ) : (
              ...
              <motion.div key={amb.id}
                ...
                <div className="px-5 py-4 flex items-center justify-between"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-sm text-white">{amb.ambulanceId || `AMB-${amb.id.slice(0, 5).toUpperCase()}`}</p>
                      <p className="text-xs" style={{ color: "#94a3b8" }}>Unit ID</p>
                    </div>
                  </div>
                  <span className="text-xs px-2.5 py-1 rounded-full font-semibold"
                    style={{ background: amb.patientOnboard ? "#fef2f2" : "#f0fdf4", color: amb.patientOnboard ? "#991b1b" : "#065f46" }}>
                    {amb.patientOnboard ? "🔴 Patient On Board" : "🟢 Available"}
                  </span>
                </div>
                {/* Body */}
                <div className="p-5 grid grid-cols-2 gap-4">
                  <InfoField icon={User} label="Driver" value={amb.driverName || "Unknown"} />
                  <InfoField icon={Clock} label="ETA to Hospital" value={amb.eta ? `${amb.eta} min` : "—"} color="#f59e0b" />
